...
# ── Model fitting ─────────────────────────────────────────────────────────────
# FIT_SIZE is the unified fit-sample parameter.
# Any model accepts fit_size=FIT_SIZE or None (fit on all available data).
# The legacy constants below preserve
...
# all labeled. Used by run_preprocessing() in preprocessor.py and the
# 1x notebooks. The Kaggle test.parquet is NOT touched by this split —
# Kaggle submissions go through run_preprocessing_kaggle() instead.
