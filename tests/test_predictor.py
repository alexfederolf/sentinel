"""Smoke tests for sentinel.ml_logic.predictor (LEGACY — commented out).

These tests exercise the legacy ``predict`` / ``predict_report`` / ``_load``
functions in ``predictor.py``, which were superseded by the FE 46-channel
pipeline (``predict_fe46`` / ``predict_fe46_report``). The legacy code is
kept commented out at the bottom of ``predictor.py`` for reference; these
tests are commented out in parallel.

To re-enable: uncomment the legacy block in ``predictor.py`` AND the
import + tests below.
"""
# import numpy as np
# import pandas as pd
